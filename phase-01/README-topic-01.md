...
*   **4.2 Nháy kép (`" "`):** Cho phép Shell vẫn thực hiện một số phép mở rộng như thay thế biến và command substitution, nhưng khoảng trắng bên trong dấu nháy kép không được dùng để tách thành nhiều đối số. Ví dụ, `echo "hello world"` sẽ truyền `hello world` cho `echo` dưới dạng một đối số duy nhất, thay vì hai đối số `hello` và `world`. Tương tự, nếu `NAME="Nguyen Van An"` thì `echo "$NAME"` sẽ truyền toàn bộ `Nguyen Van An` thành một đối số duy nhất cho `echo`.
